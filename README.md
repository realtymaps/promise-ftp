...
* Version 1.0 provides a promise-based API that mirrors the
...
* **PromiseFtp.FtpConnectionError**: Indicates the connection status was not appropriate for the method called; e.g.
**connect**() or **reconnect()** was called when the connection was not in a disconnected state, **end()** was called
when the connection has already closed, etc.

* **PromiseFtp.FtpReconnectError**: Only possible when used with the `autoReconnect` option, this indicates a
reconnect was attempted and failed.  It includes information about both the original disconnect error as well as the
error while attempting to reconnect.

In addition, errors from the underlying node-ftp library may be rejected unchanged through method calls.  In case of
protocol-level errors, the rejected error will contain a `code` property that references the related 3-digit FTP
...
ennumerated in PromiseFtp.STATUSES, as well as below:
